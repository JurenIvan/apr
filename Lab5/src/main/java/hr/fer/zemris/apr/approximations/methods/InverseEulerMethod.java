...
public class InverseEulerMethod implements Approximation {
    ...
    public InverseEulerMethod(IMatrix x0, IMatrix a, IMatrix b, IMatrix r, double t, double interval) {
        this.x0 = x0;
        this.a = a;
        this.b = b;
        this.r = r;
    ...
    public IMatrix approximate() {
        ...
        for (double i = interval; i <= t; i += interval) {
            var m = a.nMultiply(interval).add(e);
            var n = b.nMultiply(interval);
            x = m.nMultiply(x).add(n.nMultiply(r).nMultiply(x));
